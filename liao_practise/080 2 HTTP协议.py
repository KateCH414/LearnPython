...
# Chrome提供了一套完整地调试工具
# Elements显示网页的结构
# Network显示浏览器和服务器的通信：确保第一个小红灯亮着，Chrome就会记录所有浏览器和服务器之间的通信：
# 在地址栏输入www.sina.com.cn时，浏览器将显示新浪的首页。在这个过程中，浏览器都干了哪些事情呢？
# 通过Network的记录，我们就可以知道。在Network中，定位到第一条记录，点击，右侧将显示Request Headers，点击右侧的view source，我们就可以看到浏览器发给新浪服务器的请求
# 最主要的头两行分析如下，
# 第一行：GET / HTTP/1.1
...
# Web采用的HTTP协议采用了非常简单的请求-响应模式,一个HTTP请求只处理一个资源。
#   HTTP响应中把HTML发送出去，不需要考虑如何附带图片、视频等，浏览器如果需要请求图片和视频，它会发送另一个HTTP请求，
# HTTP协议同时具备极强的扩展性
#   虽然浏览器请求的是http://www.sina.com.cn/的首页，
#   但是新浪在HTML中可以链入其他服务器的资源，比如<img src="http://i1.sinaimg.cn/home/2013/1008/U8455P30DT20131008135420.png">，
#   从而将请求压力分散到各个服务器上
# 一个站点可以链接到其他站点，无数个站点互相链接起来，就形成了World Wide Web，简称“三达不溜”（WWW）
...
#   最常见的压缩方式是gzip，所以，看到Content-Encoding: gzip时，需要将Body数据先解压缩，才能得到真正的数据。
#   压缩的目的在于减少Body的大小，加快网络传输。

# 详细了解HTTP协议
# 推荐“HTTP: The Definitive Guide”一书，非常不错，有中文译本：HTTP权威指南
